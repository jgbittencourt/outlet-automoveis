...
// Sistema para adicionar veículos
let nextVehicleId = Math.max(...inventory.map(v => v.id)) + 1;
...
function openAddVehicleModal() {
  ...
  // Adicionar evento para preview de imagens
  document.getElementById('vehicleImages').addEventListener('change', handleImagePreview);
}
...
function submitAddVehicle(event) {
  ...
  // Criar array de nomes de arquivo para as imagens (se houver)
  const imageNames = files.length > 0 ? Array.from(files).map(file => file.name) : [];
  
  // Criar novo veículo
  const newVehicle = {
    id: nextVehicleId++,
    titulo: formData.get('titulo'),
    preco: parseInt(formData.get('preco')),
    anoModelo: formData.get('anoModelo'),
    km: parseInt(formData.get('km')) || 0,
    local: formData.get('local'),
    loja: formData.get('loja'),
    imagem: imageNames.length > 0 ? imageNames[0] : 'fotos/sem-imagem.svg', // Imagem padrão se não houver fotos
    imagens: imageNames.length > 0 ? imageNames : ['fotos/sem-imagem.svg']
  };
...
// Carregar inventário do localStorage se existir
function loadInventoryFromStorage() {
  const savedInventory = localStorage.getItem('vehicleInventory');
  if (savedInventory) {
    const parsedInventory = JSON.parse(savedInventory);
    // Mesclar com o inventário padrão (evitar duplicatas)
    const existingIds = inventory.map(v => v.id);
    const newVehicles = parsedInventory.filter(v => !existingIds.includes(v.id));
    inventory.push(...newVehicles);
    
    // Atualizar nextVehicleId
    nextVehicleId = Math.max(...inventory.map(v => v.id)) + 1;
  }
}

// Carregar inventário salvo ao inicializar
